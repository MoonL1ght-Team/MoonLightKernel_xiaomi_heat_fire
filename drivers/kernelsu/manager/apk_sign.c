...
#ifdef EXPECTED_MANAGER_HASHES
	const char *entry = EXPECTED_MANAGER_HASHES;

	while (*entry) {
		unsigned int size = simple_strtoul(entry, (char **)&entry, 0);
		...
		for (i = 0; i < SHA256_DIGEST_SIZE * 2 && entry[i]; i++) {
			if (entry[i] == ',' || entry[i] == ';')
				break;
			hash[i] = entry[i];
		}
		...
		if (i == SHA256_DIGEST_SIZE * 2 &&
		    check_v2_signature(path, size, hash))
			return true;
